...
def log_softmax(scores):
    ...
    numOfRows = scores.shape[0]
    numOfColumns = scores.shape[1]

    cnt = 0
    grid = np.zeros((numOfRows, numOfColumns))

    for i in scores:
        for k in i:
            x2 = np.log(np.exp(i) / sum(np.exp(i)))
            grid[cnt] = x2
...
def nll_loss(logp, y):
    ...
    out = logp[range(len(y)), y]
    negOut = -out.sum()/len(out)
...
def train_model(loss, model, X_tr_var, Y_tr_var,
                ...
                Y_dv_var = None,
                status_frequency=10,
                optim_args = {'lr':0.325,'momentum':0},
